...
vr_cobotics::vr_cobotics() 
{
	...
	label_text = "CeTI U2-TP5 Demonstrator";
	label_font_idx = 0;
	label_upright = true;
...
void vr_cobotics::draw(cgv::render::context& ctx)
{
	...
	if (movable_boxes.size() > 0)
	{
		...
		bwr.set_box_array(ctx, movable_boxes);
		bwr.set_color_array(ctx, movable_box_colors);
		bwr.set_translation_array(ctx, movable_box_translations);
		bwr.set_rotation_array(ctx, movable_box_rotations);
		if (bwr.validate_and_enable(ctx)) {
			if (show_seethrough) {
				bwr.draw(ctx, 0, movable_boxes.size());
